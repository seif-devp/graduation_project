...
class EmployerHeaderSection extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildStatCard('ACTIVE JOBS', '${data.activeJobs}', Icons.work_outline),
              ),
              SizedBox(width: 16.w),
              Expanded(
  ...
  Widget _buildStatCard(String title, String count, IconData icon, {String? trendText, bool isPositiveTrend = true}) {
    return Container(
        ...
        children: [
          Row(
            children: [
              ...
              SizedBox(width: 8.w), 
              Text(title, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w600, color: Colors.grey[600]))
            ],
          ),
          SizedBox(height: 12.h),
          Text(count, style: TextStyle(fontSize: 24.sp, fontWeight: FontWeight.bold, color: const Color(0xFF1A1C1E))),
          if (trendText != null) ...[
            SizedBox(height: 6.h),
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: (isPositiveTrend ? Colors.green : Colors.red).withOpacity(0.1), shape: BoxShape.circle),
                  child: Icon(isPositiveTrend ? Icons.trending_up : Icons.trending_down, color: isPositiveTrend ? Colors.green : Colors.red, size: 14.sp),
                ),
                SizedBox(width: 6.w),
                Text(trendText, style: TextStyle(color: isPositiveTrend ? Colors.green : Colors.red, fontSize: 12.sp, fontWeight: FontWeight.bold)),
              ],
            ),
          ],
        ],
      ...
      child: Row(
        ...
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(children: [Icon(icon, size: 16.sp, color: Colors.grey), SizedBox(width: 8.w), Text(title, style: TextStyle(color: Colors.grey[600], fontWeight: FontWeight.w600, fontSize: 14.sp))]),
              SizedBox(height: 8.h),
              Text(count, style: TextStyle(fontSize: 24.sp, fontWeight: FontWeight.bold, color: const Color(0xFF1A1C1E))),
            ],
          ),
          Text('View Calendar >', style: TextStyle(color: primaryColor, fontWeight: FontWeight.w600, fontSize: 14.sp)),
        ],
      ),
    );
  }
